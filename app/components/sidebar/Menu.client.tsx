...
const menuVariants = {
  closed: {
    opacity: 0,
    visibility: 'hidden',
    left: '-300px', // Adjusted for smaller screens
    transition: {
      duration: 0.2,
      ease: cubicEasingFn,
    },
...
function CurrentDateTime() {
  ...
  return (
    // Adjusted padding, gap, text size
    <div className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800/50">
    ...
    <>
      <motion.div
        ...
        className={classNames(
          'flex selection-accent flex-col side-menu fixed top-0 h-full w-full max-w-[300px] sm:max-w-[340px]',
          'bg-white dark:bg-gray-950 border-r border-gray-100 dark:border-gray-800/50',
          'shadow-sm text-xs sm:text-sm', // Base text size for menu
          isSettingsOpen ? 'z-40' : 'z-sidebar',
        )}
      >
        {/* Adjusted height, padding, gap, text size, icon size */}
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="p-3 space-y-2 sm:p-4 sm:space-y-3">
            <div className="flex gap-1.5 sm:gap-2"> {/* Adjusted gap */}
              <a
                href="/"
                // Adjusted padding, gap, text size
                className="flex-1 flex gap-1.5 sm:gap-2 items-center bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-500/20 rounded-lg px-3 py-1.5 sm:px-4 sm:py-2 transition-colors"
              >
                <span className="inline-block i-ph:plus-circle h-4 w-4" />
                <span className="text-xs sm:text-sm font-medium">Start new chat</span> {/* Adjusted text size */}
              </a>
              <button
                onClick={toggleSelectionMode}
                className={classNames(
                  // Adjusted padding
                  'flex gap-1 items-center rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 transition-colors',
                  selectionMode
            ...
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full bg-gray-50 dark:bg-gray-900 relative pl-8 pr-2.5 py-1.5 sm:pl-9 sm:pr-3 sm:py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-purple-500/50 text-xs sm:text-sm text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-500 border border-gray-200 dark:border-gray-800"
                type="search"
          ...
          {/* Adjusted padding, text size, and gap */}
          ...
          <div className="flex-1 overflow-auto px-2 pb-2 sm:px-3 sm:pb-3">
            ...
            <DialogRoot open={dialogContent !== null}>
              {binDates(filteredList).map(({ category, items }) => (
                <div key={category} className="mt-1.5 sm:mt-2 first:mt-0 space-y-0.5 sm:space-y-1"> {/* Adjusted margin and spacing */}
                  {/* Adjusted text size and padding for category header */}
                  <div className="text-[0.65rem] sm:text-xs font-medium text-gray-500 dark:text-gray-400 sticky top-0 z-1 bg-white dark:bg-gray-950 px-3 py-0.5 sm:px-4 sm:py-1">
                    {category}
                  </div>
                  <div className="space-y-0.5 pr-0.5 sm:pr-1"> {/* Adjusted padding */}
                    {items.map((item) => (
                      <HistoryItem
                        key={item.id}
                        item={item}
              ...
              <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                {dialogContent?.type === 'delete' && (
                  <>
                    {/* Padding is handled by Dialog component's internal structure changes */}
                    <div className="p-3 sm:p-4 md:p-6 bg-white dark:bg-gray-950">
                      <DialogTitle className="text-gray-900 dark:text-white">Delete Chat?</DialogTitle>
                      <DialogDescription className="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400"> {/* Adjusted text size */}
                        <p>
                        ...
                        <p className="mt-1 sm:mt-2">Are you sure you want to delete this chat?</p> {/* Adjusted margin */}
                      </DialogDescription>
                    </div>
                    {/* Padding and gap handled by DialogButton and Dialog structure */}
                    <div className="flex justify-end gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 md:px-6 md:py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <>
                    <div className="p-3 sm:p-4 md:p-6 bg-white dark:bg-gray-950">
                      ...
                      <DialogDescription className="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400"> {/* Adjusted text size */}
                        ...
                        {/* Adjusted margin, padding, text size */}
                        <div className="mt-1.5 sm:mt-2 max-h-28 sm:max-h-32 overflow-auto border border-gray-100 dark:border-gray-800 rounded-md bg-gray-50 dark:bg-gray-900 p-1.5 sm:p-2">
                          <ul className="list-disc pl-4 sm:pl-5 space-y-0.5 sm:space-y-1">
                            {dialogContent.items.map((item) => (
                              <li key={item.id} className="text-xs sm:text-sm"> {/* Adjusted text size */}
                                <span className="font-medium text-gray-900 dark:text-white">{item.description}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                        <p className="mt-2 sm:mt-3">Are you sure you want to delete these chats?</p> {/* Adjusted margin */}
                      </DialogDescription>
                    </div>
                     {/* Padding and gap handled by DialogButton and Dialog structure */}
                    <div className="flex justify-end gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 md:px-6 md:py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
