...
export function FileIcon({ filename, size = 'md', className }: FileIconProps) {
  ...
  const getSizeClass = (size: 'sm' | 'md' | 'lg'): string => {
    // Adjusting to meet "max w-4 h-4" requirement.
    // All sizes will now be w-4 h-4, as these are context icons.
    // If smaller is needed, a w-3 h-3 could be 'xs' if that prop is added.
    switch (size) {
      case 'sm':
        return 'w-4 h-4'; // Target size
      case 'md':
        return 'w-4 h-4'; // Target size, also default
      case 'lg':
        return 'w-4 h-4'; // Target size
      default:
        return 'w-4 h-4';
